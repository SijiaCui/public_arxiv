...
\ours adapts the Proposer--Checker paradigm~\cite{li2026march,wang2026visionzero} to the visual domain and extends it with a dedicated \textbf{Regulator} agent.
...
For the query ``Describe this image'' applied to an airport scene, the Solver generates a response $\vy$.
...
The Regulator then re-answers each question $v_i$ in $\mathcal{Q}^{\ast}$ with the
degraded image:
\begin{equation}
...
\end{equation}
where $\gF(\cdot,\cdot)$ is a binary consistency function that returns $1$ when two answers agree, $\bm{1}[\cdot]$ is the indicator function, and $\lambda_{\text{qual}}$ weights the trivial-claim penalty.
This penalty pushes the Solver toward responses that require genuine image grounding.
