...
\input{tables/main_results2.tex}
...
\input{tables/main_results3.tex}
...
Even the 3B model benefits consistently: it achieves $+8.67$ on BLINK/Jigsaw and $+2.52$ on VSR zero-shot, yielding a $+1.33$ average improvement on Mixed benchmarks.benchmarks benefit most at every scale.
...
% \input{tables/ablation_aug.tex}

\paragraph{VCR Analysis.}
\input{tables/ablation_com.tex}
...
\begin{minipage}[t]{0.60\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/training_dynamics_2.pdf}
...
\begin{minipage}[t]{0.37\textwidth}
    \centering
    \captionof{table}{Wall-clock time per training step, along with agent configuration \#Agent(trained/total) and relative overhead compared to SelfCheck with Ground Truth.}
    \label{tab:overhead}
    \vspace{2pt}
    \small
...
Its training time is therefore estimated as twice that of SelfCheck, although the actual cost may be even higher.
